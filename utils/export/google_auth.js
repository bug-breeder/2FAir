...
function decodeGoogleAuthenticatorBackupURL(urlString) {
  ...
  if (url.protocol !== "otpauth-migration:") {
    throw new Error(
      `Invalid OTP migration URL format: expected protocol to be otpauth-migration, got ${url.protocol}`
    );
  }

  if (url.host !== "offline") {
    throw new Error(
      `Invalid OTP migration URL format: expected host to be offline, got ${url.host}`
    );
  }
